...
特征维度: 7维 [E, PRXY, DENS, is_active, is_fixed, log10(K), log10(C)]
...
while valid_samples < N_SAMPLES:
    ...
    node_features[:, 3] = 1.0  # is_active
    node_features[:, 5] = -1.0  # log10(K): -1 = 无弹簧, 正值为弹簧节点
    node_features[:, 6] = -1.0  # log10(C): -1 = 无弹簧
            ...
            for n1 in clamp_nodes:
                ...
                if n1_int in node_id_to_idx and n1_int not in spring_node_set:
                    ...
                    if is_corner:
                        ...
                        node_features[idx_n1, 4] = 1.0
                        spring_info.append((n1_int, C_each, C_each, C_each))
                    else:
                        mapdl.type(3); mapdl.real(real_const_num); mapdl.e(n1_int, n2)
                        node_features[idx_n1, 4] = 0.5
                        spring_info.append((n1_int, 0.0, C_each, 0.0))

                    node_features[idx_n1, 5] = np.log10(K_each)
                    node_features[idx_n1, 6] = np.log10(C_each)
            real_const_num += 1
