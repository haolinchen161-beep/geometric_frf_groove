...
def train(args, config, model_cfg, net, dataloader, optimizer,
    ...
    # ---- 五阶段边界 ----
    _restore_phase_state(net, start_epoch, config, logger)
    ...
    if not log_exists:
        log_writer.writerow(['epoch', 'train_loss', 'omega_pct', 'zeta_pct', 'phi_pct', 'val_asinh_mse',
...
def _restore_phase_state(net, epoch, config, logger):
    """续训时恢复 freeze 状态."""
    p1 = config.get('phase1_epochs', 300)
    p2 = config.get('phase2_epochs', 600)
    if epoch == 0:
        return
    if epoch < p1:
        _set_grad(net, 'transformer', False)
        _set_grad(net, 'head_phi', False)
        _log(f"续训: 恢复阶段1 freeze", logger)
    elif epoch < p2:
        _set_grad(net, 'head_phi', False)
        _log(f"续训: 恢复阶段2 freeze", logger)
    elif epoch < config.get('phase3_epochs', 1200):
        _set_grad(net, 'head_phi', False)
        _log(f"续训: 恢复阶段3 freeze", logger)
