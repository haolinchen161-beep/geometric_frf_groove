...
└───┬──┘  └────┬─────┘
    ...
    └────┬─────┘
         │
  ┌──────┴──────┐
  │ ω skip conn │ ← softplus(skip(√E/ρ))×15000 + tanh(Δω)×10000
...
class GrooveTransFRF(nn.Module):
    ...
    def __init__(self, coord_dim=3, point_feat_dim=7,
        ...
        # ============================================
        self.skip_omega = nn.Linear(1, n_modes)
    ...
    def _compute_physics_prior(self, point_features, batch):
        """
        从逐节点特征计算物理先验 √(E/ρ)。
        对于新数据集 (L, H 固定), 简化为此形式。
        返回每样本的标量先验。

        Args:
            point_features: (total_N, 7)
            batch:          (total_N,)
        Returns:
            physics_sample: (B,) 每样本的 √(E/ρ) 先验
        """
        # E 和 ρ 在一个样本内是常量, 取第一个节点的值即可
        E_r = point_features[:, 0]    # E/E_base
        rho_r = point_features[:, 2]  # ρ/ρ_base
        physics = torch.sqrt(torch.abs(E_r / (rho_r + 1e-6)))  # (N,)

        B = int(batch.max().item()) + 1
        physics_sample = torch.zeros(B, device=point_features.device)
        for b in range(B):
            physics_sample[b] = physics[batch == b][0]
    ...
    def forward(self, geometry_data, frequencies=None, phi_exc=None):
        ...
        # 步骤4: ω 跳跃连接 + ζ 约束
        # ============================================
        physics_sample = self._compute_physics_prior(point_feat, batch)

        omega_base = self.skip_omega(physics_sample.unsqueeze(-1))[:, :self.n_modes]
