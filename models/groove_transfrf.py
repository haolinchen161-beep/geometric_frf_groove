...
class GrooveTransFRF(nn.Module):
    ...
    def __init__(self, coord_dim=3, point_feat_dim=7,
        ...
        self.macro_omega = nn.Sequential(          # 宏观流: 真物理量→ω基线
            nn.Linear(4, 64), nn.GELU(),           # 4 = √(E/ρ), avg_logK, constraint_ratio, mean_Z/H
            nn.Linear(64, n_modes)
        )
        self.micro_omega = nn.Sequential(          # 微观流: 交叉注意力特征→Δω
            nn.Linear(n_modes, 64), nn.GELU(),      # modal_out前n_modes: 已在512超节点中学会"看"关键区
            nn.Linear(64, n_modes)
        )
        # 零初始化仅最后一层: 保证初始残差=0, 但隐藏层梯度不被阻断
    ...
    def forward(self, geometry_data, frequencies=None, phi_exc=None):
        ...
        omega_fine = torch.tanh(self.micro_omega(modal_out[:, :self.n_modes])) * 5000.0  # 微观修正, 交叉注意力已选关键区
