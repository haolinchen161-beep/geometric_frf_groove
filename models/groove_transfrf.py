...
└───┬──┘  └────┬─────┘
    ...
    └────┬─────┘
         │
  ┌──────┴──────┐
  │ ω skip conn │ ← softplus(skip([√E/ρ, logK]))×15000 + tanh(Δω)×10000
...
class GrooveTransFRF(nn.Module):
    ...
    def __init__(self, coord_dim=3, point_feat_dim=7,
        ...
        # ============================================
        # 保留: ω 跳跃连接 (物理先验 → 粗 ω 基线)
        # ============================================
        self.skip_omega = nn.Linear(2, n_modes)  # 2维物理先验 → K个模态
        nn.init.constant_(self.skip_omega.weight, 0.0)
        nn.init.constant_(self.skip_omega.bias, 0.0)
    ...
    def _compute_physics_prior(self, point_features, batch):
        ...
        两个分量 (与 ω 正相关):
          1. √(E/ρ):        材料波速       ω ∝ √(E/ρ)
          2. mean_logK(bc):  边界刚度       ω ∝ √(K_structural + K_boundary)

        注意: 不包含几何分量 (mean_Z等), 因为四面体网格的Z分布
        受网格质量影响大, 与凹槽深度的相关系数仅 -0.27, 不可靠。
        几何信息由 SIREN + EdgeConv + Transformer 学习。

        Args:
            point_features: (total_N, 7) [E,PRXY,DENS,active,fix,logK,logC]
            batch:          (total_N,)
        Returns:
            physics: (B, 2) 每样本的二维物理先验
        """
        E_r = point_features[:, 0]       # E/E_base
        rho_r = point_features[:, 2]     # ρ/ρ_base
        logK = point_features[:, 5]      # log10(K), 非弹簧节点 = -1
        ...
        for b in range(B):
            ...
            # 分量1: 材料波速 (所有节点相同, 取第一个)
            ws_b = wave_speed[mask][0]

            # 分量2: 平均边界弹簧刚度 (仅弹簧节点 logK > 0)
            bc_mask = logK[mask] > 0
            if bc_mask.any():
    ...
    def forward(self, geometry_data, frequencies=None, phi_exc=None):
        ...
        omega_base = self.skip_omega(physics_prior)  # (B, n_modes)
        omega_coarse = F.softplus(omega_base) * 15000.0
        omega_fine = torch.tanh(modal_out[:, :self.n_modes]) * 10000.0
